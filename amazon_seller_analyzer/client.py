"""Thin synchronous client for the Amazon Seller Analyzer Apify Actor."""
...
from .exceptions import ActorRunError, ActorTimeoutError, AuthenticationError, AmazonSellerAnalyzerError

ACTOR_ID = "apivault_labs~amazon-seller-revenue-strategy-analyzer"
APIFY_API_BASE = "https://api.apify.com/v2"
TERMINAL_FAIL = {"FAILED", "TIMED-OUT", "ABORTED"}


class AmazonSellerAnalyzerClient:
    ...
    def __init__(self, api_token: str | None = None, timeout: int = 600,
        ...
        self.session.headers.update({
            ...
            "User-Agent": "amazon-seller-analyzer-python/0.1.0",
        })
    ...
    def _start(self, payload: dict[str, Any], timeout_secs: int) -> str:
        try:
            ...
            raise AmazonSellerAnalyzerError(f"Could not start Actor run: {exc}") from exc
    ...
    def _wait(self, run_id: str) -> dict[str, Any]:
        ...
        while True:
            try:
                ...
                raise AmazonSellerAnalyzerError(f"Could not poll Actor run: {exc}") from exc
            if response.status_code >= 400:
    ...
    def _dataset(self, dataset_id: str) -> list[dict[str, Any]]:
        try:
            ...
            raise AmazonSellerAnalyzerError(f"Could not download Dataset: {exc}") from exc
        if response.status_code >= 400:
